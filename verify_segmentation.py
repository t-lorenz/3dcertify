...
from util.math import DEFAULT_SEED, logits_to_category, set_random_seed
...
parser.add_argument(
    "--experiment",
    type=str,
    help="Unique identifier name for the experiment. Will be generated by default",
)
parser.add_argument(
    "--theta",
    type=parse_theta,
...
parser.add_argument("--intervals",
                    type=int,
                    default=1,
                    help="Number of parameter splits per dimension")
parser.add_argument(
    "--transformation",
    type=str,
    default="RotationZ",
    help="The transformation function, or composition of multiple transformations separated by '+'",
)
parser.add_argument(
    "--pooling",
    choices=["improved_max"],
...
parser.add_argument(
    "--relaxation",
    default="taylor",
    choices=["taylor", "interval"],
    help="The relaxation to use",
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of implicit splits for interval bound refinement",
)
...
if len(parameters) != transformation.num_params:
    raise ValueError(
        f"The number of parameters does no match those required by the transformation. Expected {transformation.num_params}, received {parameters}"
    )
...
for counter, i in enumerate(range(0, test_samples, interval)):
    ...
    theta_inc = [
        2.0 * param / float(intervals)
        for param, intervals in zip(parameters, intervals_per_dimension)
    ]
            ...
            if settings.relaxation == "interval":
                ...
                if settings.implicit_intervals > 1:
                    bounds = split_implicitly(
                        bounds=bounds,
                        intervals=settings.implicit_intervals,
                        encode_rotation=partial(transformation.transform,
                                                np_points),
                        params=params,
                    )

                (dominant_classes, nlb,
                 nub) = eran.analyze_segmentation_box(bounds, label,
                                                      valid_classes,
                                                      num_total_classes)
                ...
                (dominant_classes, nlb,
                 nub) = eran.analyze_segmentation_linear(
    ...
    logger.info(
        f"Successfully certified {percentage_certified} of points in {settings.intervals} intervals"
    )
