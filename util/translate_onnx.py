...
def expand_conv_node(node: onnx.NodeProto) -> onnx.NodeProto:
    ...
    for attribute in node.attribute:
        if attribute.name == "dilations":
            ...
            assert False, "Unsupported Attribute in MaxPool layer: {}".format(
                attribute)

    return onnx.helper.make_node(node.op_type, node.input, node.output,
                                 **attributes)
...
def expand_pool(node: onnx.NodeProto) -> onnx.NodeProto:
    ...
    for attribute in node.attribute:
        if attribute.name == "kernel_shape":
            ...
            assert False, "Unsupported Attribute in pooling layer: {}".format(
                attribute)
...
def expand_tensor(tensor: onnx.TensorProto, axis: int) -> onnx.TensorProto:
    dims = tensor.dims
    dims.insert(axis, 1)
    return onnx.helper.make_tensor(tensor.name,
                                   tensor.data_type,
                                   dims,
                                   tensor.raw_data,
                                   raw=True)
...
def expand_input(input: onnx.ValueInfoProto, axis: int) -> onnx.ValueInfoProto:
    ...
    shape.insert(axis, 1)
    return onnx.helper.make_tensor_value_info(input.name,
...
def merge_batch_norm(affine_node: onnx.NodeProto, bn_node: onnx.NodeProto,
    ...
    for i, tensor in enumerate(graph.initializer):
        ...
        # bias tensor
            ...
            fused_bias = bn_shift + (
                conv_bias - bn_mean) * bn_scale / np.sqrt(bn_var + eps)
...
def merge_bn_layers(model: onnx.ModelProto):
    ...
    to_remove = []
    for i, node in enumerate(graph.node):
        if node.op_type == "BatchNormalization" and graph.node[i -
                                                               1].op_type in [
                                                                   "Conv",
                                                                   "Gemm",
        ]:
            affine_node = graph.node[i - 1]
            assert (affine_node.output[0] == node.input[0]
                    ), "Consecutive Conv and BN nodes are not connected"
    ...
    for i, node in enumerate(graph.node):
        if node.op_type == "Conv":
            ...
            if graph.node[i + 1].op_type == "Tile":
                ...
                new_tensor = onnx.helper.make_tensor(tensor.name,
                                                     tensor.data_type, (4, ),
                                                     new_value,
                                                     raw=False)
                new_attribute = onnx.helper.make_attribute(
                    attribute.name, new_tensor)
...
def remove_first_affine(model: onnx.ModelProto):
    ...
    identity_weight = from_array(
        np.expand_dims(np.eye(64, dtype=np.float32), axis=(2, 3)),
        "identity_weight")
    identity_bias = from_array(np.zeros(64, dtype=np.float32), "identity_bias")
    identity_node = onnx.helper.make_node(
        op_type="Conv",
        inputs=[
            first_affine.input[0], identity_weight.name, identity_bias.name
        ],
    ...
    new_input = onnx.helper.make_tensor_value_info(
        input.name, input.type.tensor_type.elem_type, shape)
    graph.input.remove(input)
...
if __name__ == "__main__":
    ...
    session = onnxruntime.InferenceSession(
        "out/face_propagation_test/cls_model_249.onnx")
    inputs = {session.get_inputs()[0].name: dummy_input.copy()}
    original_outputs = session.run(None, inputs)

    session = onnxruntime.InferenceSession(model.SerializeToString())
    inputs = {
        session.get_inputs()[0].name: np.expand_dims(dummy_input.copy(),
                                                     axis=-1)
    }
    ...
    print(
        "max abs difference: ",
        np.max(
            np.abs(np.array(original_outputs) -
                   np.array(transformed_outputs))),
    )
    print(
        "mean abs difference:",
        np.mean(
