...
def random_rotate_z(points: np.ndarray,
                    theta_min: float = -np.pi,
                    theta_max: float = np.pi) -> np.ndarray:
...
def rotate_z_batch(points: torch.Tensor, theta: float) -> torch.Tensor:
    rotation_matrix = (torch.from_numpy(rotation_matrix_z(theta).T).float().to(
        points.device))
    return torch.matmul(points, rotation_matrix)


def random_rotate_so3_batch(points: torch.Tensor,
                            theta_min: float = -np.pi,
                            theta_max: float = np.pi) -> torch.Tensor:
    np_points = points.cpu().numpy()
    rotated = np.array([
        random_rotate_so3(np_points[i], theta_min, theta_max)
        for i in range(np_points.shape[0])
    ])
    return torch.from_numpy(rotated).float().to(points.device)


def random_rotate_z_batch(points: torch.Tensor,
                          theta_min: float = -np.pi,
                          theta_max: float = np.pi) -> torch.Tensor:
    np_points = points.cpu().numpy()
    rotated = np.array([
        random_rotate_z(np_points[i], theta_min, theta_max)
        for i in range(np_points.shape[0])
    ])
    return torch.from_numpy(rotated).float().to(points.device)
