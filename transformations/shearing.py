...
class ShearingZ(Transformation):
    ...
    def transform(
        ...
        return iv.stack(
            ...
            axis=1,
            convert=isinstance(sx, Interval),
        )
    ...
    def gradient_params(
        ...
        return [
            iv.stack([z, zero, zero], axis=1, convert=isinstance(sx,
                                                                 Interval)),
            iv.stack([zero, z, zero], axis=1, convert=isinstance(sx,
                                                                 Interval)),
        ]
    ...
    def gradient_points(
        ...
        one = iv.ones_like(z)
        return [
            iv.stack([one, zero, zero],
                     axis=1,
                     convert=isinstance(sx, Interval)),
            iv.stack([zero, one, zero],
                     axis=1,
                     convert=isinstance(sx, Interval)),
            iv.stack([sx * one, sy * one, one],
                     axis=1,
                     convert=isinstance(sx, Interval)),
        ]
    ...
    def hessian_params(
        ...
        zero = (iv.as_interval(iv.zeros_like(points)) if isinstance(
            params[0], Interval) else iv.zeros_like(points))
