...
class TaperingRotation(Transformation):
    ...
    def transform(
        ...
        a, b, theta = params
        return TaperingZ().transform(RotationZ().transform(points, [theta]),
                                     [a, b])
    ...
    def hessian_points_params(
        ...
        return [
            [
                iv.stack(
                    [a * z * iv.cos(theta), a * z * iv.sin(theta), zero],
                    axis=1,
                    convert=isinstance(a, Interval),
                ),
                iv.stack(
                    [z * iv.cos(theta), z * iv.sin(theta), zero],
                    axis=1,
                    convert=isinstance(a, Interval),
                ),
                iv.stack(
            ...
            [
                iv.stack(
                    [-a * z * iv.sin(theta), a * z * iv.cos(theta), zero],
                    axis=1,
                    convert=isinstance(a, Interval),
                ),
                iv.stack(
                    [-z * iv.sin(theta), z * iv.cos(theta), zero],
                    axis=1,
                    convert=isinstance(a, Interval),
                ),
                iv.stack(
...
class RotationZX(Transformation):
    ...
    def transform(
        ...
        return iv.stack(
            [
                iv.cos(a) * x - iv.sin(a) * iv.cos(b) * y +
                iv.sin(a) * iv.sin(b) * z,
                iv.sin(a) * x + iv.cos(a) * iv.cos(b) * y -
                iv.cos(a) * iv.sin(b) * z,
    ...
    def gradient_points(
        ...
        return [
            iv.stack(
                [iv.cos(a) * one, iv.sin(a) * one, zero],
                axis=1,
                convert=isinstance(a, Interval),
            ),
            iv.stack(
    ...
    def hessian_points(
        ...
        a, b = params
        zero = (iv.as_interval(iv.zeros_like(points)) if isinstance(
            a, Interval) else iv.zeros_like(points))
    ...
    def hessian_points_params(
        ...
        return [
            [
                iv.stack(
                    [-iv.sin(a) * one, iv.cos(a) * one, zero],
                    axis=1,
                    convert=isinstance(a, Interval),
                ),
                iv.stack([zero, zero, zero],
