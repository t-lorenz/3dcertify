...
class Composition(Transformation):
    ...
    def transform(
        ...
        return self.outer.transform(
            self.inner.transform(points, params[self.outer.num_params:]),
            params[:self.outer.num_params],
        )
    ...
    def gradient_params(
        ...
        d_outer_d_points = self.outer.gradient_points(transformed_inner,
                                                      params_outer)
        ...
        for inner_derivative in d_inner_d_params:
            ...
            for i in range(NUM_COORDINATES):
                derivative = (
                    derivative +
                    d_outer_d_points[i] * inner_derivative[:, i, np.newaxis])
    ...
    def hessian_params(
        ...
        result: Union[List[List[np.ndarray]],
                      List[List[Interval]]] = [[None] * self.num_params
                                               for _ in range(self.num_params)]

        transformed_inner = self.inner.transform(points, params_inner)
        # (d_theta_i x d_theta_j x num_points x coords)
        outer_gradient_points = self.outer.gradient_points(
            transformed_inner, params_outer)
        outer_hessian_params = self.outer.hessian_params(
            transformed_inner, params_outer)
        outer_hessian_points = self.outer.hessian_points(
            transformed_inner, params_outer)
        outer_hessian_points_params = self.outer.hessian_points_params(
            transformed_inner, params_outer)
        inner_gradient_params = self.inner.gradient_params(
            points, params_inner)
        inner_gradient_points = self.inner.gradient_points(
            points, params_inner)
        ...
        for i in range(self.inner.num_params):
            for j in range(self.inner.num_params):
                ...
                for k in range(NUM_COORDINATES):
                    derivative = (derivative + outer_gradient_points[k] *
                                  inner_hessian_params[i][j][:, k, np.newaxis])
                for k in range(NUM_COORDINATES):
                    for l in range(NUM_COORDINATES):
                        derivative = (
                            derivative + outer_hessian_points[k][l] *
                            inner_gradient_points[i][:, k, np.newaxis] *
                            inner_gradient_points[j][:, l, np.newaxis])
                result[self.outer.num_params + i][self.outer.num_params +
                                                  j] = derivative
            ...
            for j in range(self.inner.num_params):
                ...
                for k in range(NUM_COORDINATES):
                    derivative = (derivative +
                                  outer_hessian_points_params[k][i] *
                                  inner_gradient_params[j][:, k, np.newaxis])
    ...
    def hessian_points(
        ...
        outer_gradient_points = self.outer.gradient_points(
            inner_transformed, params_outer)
        ...
        result: Union[List[List[np.ndarray]],
                      List[List[Interval]]] = [[None] * NUM_COORDINATES
        ...
        for i in range(NUM_COORDINATES):
            for j in range(NUM_COORDINATES):
                ...
                for k in range(NUM_COORDINATES):
                    derivative = (derivative + outer_gradient_points[k] *
                                  inner_hessian_points[i][j][:, k, np.newaxis])
                for k in range(NUM_COORDINATES):
                    for l in range(NUM_COORDINATES):
                        derivative = (
                            derivative + outer_hessian_points[k][l] *
    ...
    def hessian_points_params(
        ...
        inner_gradient_points = self.inner.gradient_points(
            points, params_inner)
        inner_hessian_points_params = self.inner.hessian_points_params(
            points, params_inner)
        ...
        for i in range(NUM_COORDINATES):
            for j in range(self.outer.num_params):
                ...
                for k in range(NUM_COORDINATES):
                    derivative = (derivative +
                                  outer_hessian_points_params[k][j] *
                                  inner_gradient_points[i][:, k, np.newaxis])
                result[i][j] = derivative
        ...
        for i in range(NUM_COORDINATES):
            for j in range(self.inner.num_params):
                ...
                for k in range(NUM_COORDINATES):
                    derivative = (
                        derivative + outer_gradient_points[k] *
                        inner_hessian_points_params[i][j][:, k, np.newaxis])
                for k in range(NUM_COORDINATES):
                    for l in range(NUM_COORDINATES):
                        derivative = (
                            derivative + outer_hessian_points[k][l] *
                            inner_gradient_points[i][:, k, np.newaxis] *
                            inner_gradient_params[j][:, l, np.newaxis])
                result[i][self.outer.num_params + j] = derivative
        return result
