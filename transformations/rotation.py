...
class RotationX(Transformation):
    ...
    def gradient_points(
        ...
        one = iv.ones_like(x)
        d_x = iv.stack([one, zero, zero],
                       axis=1,
                       convert=isinstance(alpha, Interval))
        d_y = iv.stack(
            [zero, iv.cos(alpha) * one,
             iv.sin(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
        d_z = iv.stack(
            [zero, -iv.sin(alpha) * one,
             iv.cos(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
    ...
    def hessian_params(
        ...
        return [[
            iv.stack(
                [d_alpha_alpha_x, d_alpha_alpha_y, d_alpha_alpha_z],
                axis=1,
                convert=isinstance(alpha, Interval),
            )
        ]]
    ...
    def hessian_points_params(
        ...
        d_za = iv.stack(
            [zero, -iv.cos(alpha) * one, -iv.sin(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
...
class RotationY(Transformation):
    ...
    def gradient_points(
        ...
        one = iv.ones_like(x)
        d_x = iv.stack(
            [iv.cos(alpha) * one, zero, -iv.sin(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
        d_y = iv.stack([zero, one, zero],
                       axis=1,
                       convert=isinstance(alpha, Interval))
        d_z = iv.stack(
            [iv.sin(alpha) * one, zero,
             iv.cos(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
    ...
    def hessian_points_params(
        ...
        one = iv.ones_like(x)
        d_xa = iv.stack(
            [-iv.sin(alpha) * one, zero, -iv.cos(alpha) * one],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
...
class RotationZ(Transformation):
    ...
    def transform(
        self,
        points: Union[np.ndarray, Interval],
        params: Union[List[float], List[Interval]],
    ) -> Union[np.ndarray, Interval]:
        assert len(params) == 1
        alpha = params[0]
        x = points[:, 0]
        y = points[:, 1]
    ...
    def gradient_points(
        ...
        d_y = iv.stack(
            [-iv.sin(alpha) * one,
             iv.cos(alpha) * one, zero],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
        d_z = iv.stack([zero, zero, one],
                       axis=1,
                       convert=isinstance(alpha, Interval))
        return [d_x, d_y, d_z]
    ...
    def hessian_points_params(
        ...
        d_ya = iv.stack(
            [-iv.cos(alpha) * one, -iv.sin(alpha) * one, zero],
            axis=1,
            convert=isinstance(alpha, Interval),
        )
