...
class RelaxationTestCase(unittest.TestCase):
    ...
    def assertSound(
        self,
        approximation: Union[Interval, LinearBounds],
    ...
    def assertSoundInterval(self, approximation: Interval,
                            params: List[Interval], function):
        samples = [[
            np.random.uniform(p.lower_bound, p.upper_bound) for p in params
        ] for _ in range(1000)]
        ...
        for sampled_params in samples:
            ...
            self.assertAlmostLessThan(concrete_values,
                                      approximation.lower_bound)
            self.assertAlmostGreaterThan(concrete_values,
                                         approximation.upper_bound)

    def assertSoundBounds(self, approximation: LinearBounds,
                          params: List[Interval], function):
        samples = [[
            np.random.uniform(p.lower_bound, p.upper_bound) for p in params
        ] for _ in range(1000)]
        ...
        for sampled_params in samples:
            ...
            for j, p0 in enumerate(sampled_params):
                lower_bound = lower_bound + approximation.lower_slope[:, :,
                                                                      j] * p0
                upper_bound = upper_bound + approximation.upper_slope[:, :,
                                                                      j] * p0
    ...
    def assertAlmostGreaterThan(self, expected: np.ndarray,
                                actual: np.ndarray):
        tolerance = ABSOLUTE_TOLERANCE + RELATIVE_TOLERANCE * np.abs(expected)
        # only enable for debugging as it will significantly increase test runtime
        # self.assertTrue(np.all(actual + tolerance >= expected), f"Max difference: {np.max(expected - actual)}, tolerance: {tolerance}")
    ...
    def assertAlmostEqualBounds(self, expected: LinearBounds,
                                actual: LinearBounds):
        ...
        self.assertAlmostEqualNumpy(expected.lower_slope.flatten(),
                                    actual.lower_slope.flatten())
        self.assertAlmostEqualNumpy(expected.upper_slope.flatten(),
                                    actual.upper_slope.flatten())


def sample_intervals(num_batches=100, size_intervals=100):
    scaling = (np.linspace(0, 1, num_batches).reshape(
        (num_batches, 1)).repeat(size_intervals, axis=1))
    center = np.random.uniform(-100 * scaling, 100 * scaling,
                               (num_batches, size_intervals)).squeeze()
    width = np.random.uniform(0, 200 * scaling,
                              (num_batches, size_intervals)).squeeze()
    return [
        Interval(center[i] - width[i], center[i] + width[i])
...
def sample_params(num_params=1, num_batches=100):
    scaling = (np.linspace(0, 1, num_batches).reshape(
        (num_batches, 1)).repeat(num_params, axis=1))
    center = np.random.uniform(-100 * scaling, 100 * scaling,
                               (num_batches, num_params))
    width = np.random.uniform(0, 4 * scaling, (num_batches, num_params))
    return [[
        Interval(center[i, j] - width[i, j], center[i, j] + width[i, j])
...
def sample_points(num_batches=100, num_points=100):
    scaling = (np.linspace(0, 1, num_batches).reshape(
        (num_batches, 1, 1)).repeat(num_points, axis=1).repeat(3, axis=2))
    return np.random.uniform(-100 * scaling, 100 * scaling,
                             (num_batches, num_points, 3))
