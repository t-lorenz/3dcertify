...
class TNet(nn.Module):
    ...
    def __init__(self, number_points, num_features):
        ...
        self.mlp1 = nn.Sequential(
            nn.Conv1d(in_channels=num_features, out_channels=64,
                      kernel_size=1),
    ...
    def forward(self, x):
        ...
        identity = identity.expand(batch_size, self.num_features,
                                   self.num_features)
...
class PointNet(nn.Module):
    ...
    def __init__(
        ...
        assert (number_points % 8 == 0
                ), f"Number of points must be divisible by 8: {number_points}"
        ...
        self.mlp2 = nn.Sequential(
            ...
            nn.Conv1d(in_channels=128,
                      out_channels=self.max_features,
                      kernel_size=1),
        ...
        if pool_function == "improved_max":
            ...
            while remaining_dim > 8:
                assert (
                    remaining_dim %
                    8 == 0), "number_points must be recursively divisible by 8"
                pooling.append(nn.MaxPool1d(kernel_size=8, stride=8))
                remaining_dim = remaining_dim // 8
            pooling.append(
                nn.MaxPool1d(kernel_size=remaining_dim, stride=remaining_dim))
    ...
    def forward(self, x):
        if not self.disable_assertions and not self.transposed_input:
            assert (
                x.dim() == 3
            ), f"Expect input with 3 dimensions: (batch x num_points x features), got {x.size()}."
            assert (
                x.size(1) == self.number_points
            ), f"Expect input of size (N x num_points x features), got {x.size()}."
            assert (
                x.size(2) == 3
            ), f"Expect input of size (N x num_points x features), got {x.size()}."
    ...
    def __init__(self, number_points, num_classes, pool_function="max"):
        ...
        if pool_function == "max":
            self.pooling = nn.Sequential(
                nn.MaxPool1d(kernel_size=self.number_points,
                             stride=self.number_points))
        elif pool_function == "avg":
            self.pooling = nn.Sequential(
                nn.AvgPool1d(kernel_size=self.number_points,
                             stride=self.number_points))
        else:
