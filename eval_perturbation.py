...
if __name__ == "__main__":
    ...
    parser.add_argument("--eps",
                        type=float,
                        default=0.01,
                        help="radius of box around points to attack")
    parser.add_argument(
        "--eps_step",
        type=float,
        default=None,
        help="step size of pgd attack, default is eps/2",
    )
    parser.add_argument("--fgsm_iter",
                        type=int,
                        default=50,
                        help="iterations of fgsm for pgd attack")
    parser.add_argument(
        "--max_features",
        type=int,
        default=1024,
    ...
    for i, data in enumerate(tqdm(test_loader)):
        ...
        correct, predictions = evaluate_bgd(model, domain, label,
                                            settings.eps_step,
                                            settings.fgsm_iter)
    ...
    logger.info(
        "Test Accuracy: Base: {base_accuracy}, Vote: {vote_accuracy}, BGD: {pgd_accuracy}"
        .format(
