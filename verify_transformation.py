...
from util.timing import BOUND_TIMER, TOTAL_TIMER, Timer
...
for counter, i in enumerate(range(0, test_samples, interval)):
    ...
    if not correct:
        logger.info(
            f"Incorrect prediction, skipping. True label was {label}, prediction was {max_prediction}"
        )
        continue
    ...
    for idx in progress_bar:
        ...
        if interval_key in checkpoints_sample:
            ...
            if settings.relaxation == "interval":
                ...
                (dominant_class, nlb,
                 nub) = eran.analyze_classification_box(bounds)
                ...
                (dominant_class, nlb,
                 nub) = eran.analyze_classification_linear(
                     bounds, constraints, params)
    ...
    if certified:
        logger.info(
            f"Successfully certified class {label.item()} in {settings.intervals} intervals"
        )
        verified_same += 1
    else:
    ...
    logger.info(
        f"Time for this round: (bounds: {elapsed_bounds}s) (total: {elapsed_total}s). Total time: (bounds: {timer.get(BOUND_TIMER, 0.0)}s) (total: {timer.get(TOTAL_TIMER)}s)."
    )
    logger.info(
