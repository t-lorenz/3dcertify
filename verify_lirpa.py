...
from util.argparse import absolute_path, parse_theta
from util.experiment import Experiment
from util.math import DEFAULT_SEED, set_random_seed
...
parser.add_argument(
    ...
    type=parse_theta,
    required=True,
    help="List of transformation parameters to certify. Either number or number followed by 'deg' for degree.",
)
...
lirpa_model = BoundedModule(torch_model, torch.empty(2, settings.num_points,
                                                     3))
lirpa_model.eval()
...
for i in tqdm(range(0, test_samples, interval)):
    ...
    assert torch.all(
        lirpa_prediction == prediction
    ), f"Lirpa prediction differs from torch prediction! Torch: {prediction}, LiRPA: {lirpa_prediction}"

    if not correct:
        logger.info(
            "Incorrect prediction, skipping. True label was {}, prediction was {}"
            .format(true_label, max_prediction))
        continue

    correct_predictions += 1

    labels = torch.tensor([true_label])
    c = torch.eye(num_class).type_as(points)[labels].unsqueeze(1) - torch.eye(
        num_class).type_as(points).unsqueeze(0)
    ...
    method = "backward"
    lb, ub = lirpa_model.compute_bounds(x=(relaxed_points, ),
                                        method=method,
                                        C=c,
                                        bound_upper=False)
    ...
    logger.info(
        f"Successfully verified {verified_same} samples, unable to verify {not_verified} samples. "
