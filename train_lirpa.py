...
parser.add_argument("--verify",
                    action="store_true",
                    help="verification mode, do not train")
...
parser.add_argument("--eps",
                    type=float,
                    default=0.01,
                    help="Target training epsilon")
parser.add_argument(
    "--bound_type",
    type=str,
    default="CROWN-IBP",
    choices=["IBP", "CROWN-IBP", "CROWN", "CROWN-FAST"],
...
parser.add_argument("--conv_mode",
                    type=str,
                    choices=["matrix", "patches"],
                    default="matrix")
...
def Train(model,
    ...
    for i, (data, _, labels) in enumerate(loader):
        ...
        ptb = PerturbationLpNorm(norm=np.inf,
                                 eps=eps,
                                 x_L=data_lb,
                                 x_U=data_ub)
...
def main(args):
    ...
    model_ori = PointNet(number_points=args.num_points,
                         num_classes=40,
    ...
    train_data = datasets.modelnet40(num_points=args.num_points,
                                     split="train",
                                     rotate="z")
    test_data = datasets.modelnet40(num_points=args.num_points,
                                    split="test",
                                    rotate="none")
    ...
    if args.verify:
        ...
        for t in range(1, args.num_epochs + 1):
            ...
            torch.save(
                model.state_dict(),
                args.save_model if args.save_model != "" else args.model,
            )
