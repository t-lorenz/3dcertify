...
class Interval:
    ...
    def __repr__(self):
        return (
            f"Interval(lower_bound={self.lower_bound}, upper_bound={self.upper_bound})"
        )
    ...
    def __getitem__(self, item):
        return Interval(self.lower_bound.__getitem__(item),
...
def square(x: [Interval, float, np.ndarray]):
    if isinstance(x, Interval):
        ...
        upper_bound = np.maximum(np.square(x.lower_bound),
                                 np.square(x.upper_bound))
        # Special case where all values are positive
        lower_bound = np.where(x.lower_bound >= 0, np.square(x.lower_bound),
                               lower_bound)
        upper_bound = np.where(x.lower_bound >= 0, np.square(x.upper_bound),
                               upper_bound)
        # Special case where all values are negative
        lower_bound = np.where(x.upper_bound <= 0, np.square(x.upper_bound),
                               lower_bound)
        upper_bound = np.where(x.upper_bound <= 0, np.square(x.lower_bound),
                               upper_bound)
...
def sin(theta: [Interval, float, np.ndarray]):
    if isinstance(theta, Interval):
        ...
        lower = np.where(
            np.logical_and(theta_lower <= 3 * PI_HALF,
                           3 * PI_HALF <= theta_upper),
            -1,
            lower,
        )
        upper = np.where(
            np.logical_and(theta_lower <= PI_HALF, PI_HALF <= theta_upper), 1,
            upper)
        lower = np.where(
            np.logical_and(theta_lower <= 7 * PI_HALF,
                           7 * PI_HALF <= theta_upper),
            -1,
            lower,
        )
        upper = np.where(
            np.logical_and(theta_lower <= 5 * PI_HALF,
                           5 * PI_HALF <= theta_upper),
            1,
            upper,
        )
...
def stack(elements: Iterable[Union[Interval, np.ndarray]],
          axis: int,
          convert=False):
...
def encode_rotation_box2d(points: np.ndarray, alpha: Interval,
                          beta: Interval) -> Interval:
    sin_a = sin(alpha)
    cos_a = cos(alpha)
    sin_b = sin(beta)
    cos_b = cos(beta)
...
def encode_rotation_box3d(points: np.ndarray, alpha: Interval, beta: Interval,
                          gamma: Interval) -> Interval:
    sin_a = sin(alpha)
    cos_a = cos(alpha)
    sin_b = sin(beta)
    cos_b = cos(beta)
...
if __name__ == "__main__":
    for i in range(1000):
        ...
        for j in range(100):
            ...
            y = points[:, 1]
            z = points[:, 2]
            x_rotated = (x * (np.cos(alpha0)) + y *
                         (-np.sin(alpha0) * np.cos(beta0)) + z *
                         (np.sin(alpha0) * np.sin(beta0)))
            y_rotated = (x * (np.sin(alpha0)) + y *
                         (np.cos(alpha0) * np.cos(beta0)) + z *
                         (-np.cos(alpha0) * np.sin(beta0)))
            z_rotated = y * np.sin(beta0) + z * np.cos(beta0)
            rotated_points = np.stack([x_rotated, y_rotated, z_rotated],
                                      axis=1)
    ...
    for i in range(1000):
        ...
        for j in range(100):
            ...
            x_rotated = (x * (np.cos(alpha0) * np.cos(beta0)) + y *
                         (np.cos(alpha0) * np.sin(beta0) * np.sin(gamma0) -
                          np.sin(alpha0) * np.cos(gamma0)) + z *
                         (np.cos(alpha0) * np.sin(beta0) * np.cos(gamma0) +
                          np.sin(alpha0) * np.sin(gamma0)))
            y_rotated = (x * (np.sin(alpha0) * np.cos(beta0)) + y *
                         (np.sin(alpha0) * np.sin(beta0) * np.sin(gamma0) +
                          np.cos(alpha0) * np.cos(gamma0)) + z *
                         (np.sin(alpha0) * np.sin(beta0) * np.cos(gamma0) -
                          np.cos(alpha0) * np.sin(gamma0)))
            z_rotated = (x * (-np.sin(beta0)) + y *
                         (np.cos(beta0) * np.sin(gamma0)) + z *
