...
class LinearBounds:
    ...
    def __add__(self, other):
        if isinstance(other, LinearBounds):
            return LinearBounds(
                self.upper_slope + other.upper_slope,
            ...
            return LinearBounds(
                self.upper_slope,
                self.upper_offset + other.upper_bound,
                self.lower_slope,
                self.lower_offset + other.lower_bound,
            )
        else:
            return LinearBounds(
                self.upper_slope,
                self.upper_offset + other,
                self.lower_slope,
                self.lower_offset + other,
            )
    ...
    def __mul__(self, other):
        ...
        return LinearBounds(
            ...
            upper_offset=np.where(other < 0.0, other * self.lower_offset,
                                  other * self.upper_offset),
            lower_slope=np.where(
                other_reshaped < 0.0,
                other_reshaped * self.upper_slope,
                other_reshaped * self.lower_slope,
            ),
            lower_offset=np.where(other < 0.0, other * self.upper_offset,
                                  other * self.lower_offset),
        )

    def __rmul__(self, other):
        if other < 0.0:
            return LinearBounds(
                other * self.lower_slope,
                other * self.lower_offset,
                other * self.upper_slope,
                other * self.upper_offset,
            )
        else:
            return LinearBounds(
    ...
    def __repr__(self):
        return (
            f"LinearBounds(upper_slope: {self.upper_slope}, upper_offset: {self.upper_offset}, "
            f"lower_slope: {self.lower_slope}, lower_offset: {self.lower_offset})"
        )

    def evaluate_at(self, x) -> Interval:
        return Interval(
            self.lower_offset + self.lower_slope[:, :, 0] * x,
            self.upper_offset + self.upper_slope[:, :, 0] * x,
        )

    def evaluate_at_2d(self, x, y) -> Interval:
        return Interval(
            lower_bound=self.lower_offset + self.lower_slope[:, :, 0] * x +
            self.lower_slope[:, :, 1] * y,
            upper_bound=self.upper_offset + self.upper_slope[:, :, 0] * x +
            self.upper_slope[:, :, 1] * y,
        )
