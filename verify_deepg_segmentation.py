...
from util.math import DEFAULT_SEED, logits_to_category
...
for counter, i in enumerate(range(0, test_samples, interval)):
    ...
    for params, bounds, constraints in progress_bar:
        ...
        if interval_key in checkpoints_sample:
            ...
            transformed_points = transformation.transform(
                np_points,
                [(i.lower_bound + i.upper_bound) / 2.0 for i in params])

            assert (np.min(bounds.upper_bound - transformed_points.flatten())
                    >= -0.0001), "Upper bound violation!"
            assert (np.min(transformed_points.flatten() - bounds.lower_bound)
                    >= -0.0001), "Lower bound violation!"

            (dominant_classes, nlb, nub) = eran.analyze_segmentation_linear(
                bounds, constraints, params, label, valid_classes,
                num_total_classes)
            dominant_classes = np.array(dominant_classes)

            assert np.all(
                np.logical_or(label == dominant_classes,
                              dominant_classes == -1)
    ...
    logger.info(
        f"Tested {iterations} data points for which on average {total_percentage_certified / iterations} of points were correctly predicted."
    )
